  ...
  return (
    <header className="fixed top-6 inset-x-0 z-50 px-4">
      <div className="max-w-[1280px] mx-auto flex items-center justify-center gap-3">
        ...
        {/* ЦЕНТРАЛЬНАЯ КАРТОЧКА: ПРОГРЕСС + ТЕМА (компактная) */}
        <div className={`${cardBase} shrink-0 p-1 flex items-center gap-1`}>
          <div
          ...
          >
            <svg className="-rotate-90 w-full h-full" viewBox="0 0 32 32" aria-hidden="true">
              <circle cx="16" cy="16" r={radius} className={isDark ? 'stroke-white/10' : 'stroke-black/5'} strokeWidth="2.5" fill="transparent" />
              <motion.circle
                cx="16"
                cy="16"
                r={radius}
          ...
          <button
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 hover:scale-105 active:scale-95 ${ghostBtn}`}
          >
